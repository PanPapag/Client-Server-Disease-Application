...
bucket_ptr __bucket_create(hash_table_ptr hash_table, size_t bucket_entries) {
  ...
  if (bucket == NULL) {
    die("Could not allocate memory for Hash Table Bucket. Exiting...");
  }
  bucket->bucket_size_ = 0U;
  bucket->table_ = __MALLOC__(hash_table->bucket_entries_ , bucket_entry_t);
  if (bucket->table_ == NULL) {
    die("Could not allocate memory for Hash Table Bucket Table. Exiting...");
  }
...
hash_table_ptr hash_table_create(size_t ht_entries, size_t bucket_size,
  ...
  hash_table->table_ = __MALLOC__(ht_entries, list_ptr);
  if (hash_table->table_ == NULL) {
    die("Could not allocate memory for Hash Table Table. Exiting...");
  }
