...
statistics_entry_ptr statistics_entry_create(const char *file_name,
  ...
  if (statistics_entry == NULL) {
    die("Could not allocate memory for Statistics Entry. Exiting...");
  }
  /* Allocate memory for statistics_entry file_name member */
  statistics_entry->file_name = __MALLOC__(strlen(file_name)+1, char);
  if (statistics_entry->file_name == NULL) {
    die("Could not allocate memory for Statistics Entry file_path member. Exiting...");
  }
  strcpy(statistics_entry->file_name, file_name);
  /* Allocate memory for statistics_entry country member */
  statistics_entry->country = __MALLOC__(strlen(country)+1, char);
...
void ptr_to_statistics_entry_destroy(void *v) {
  ...
  if (v != NULL) {
    __FREE__(statistics_entry->file_name);
    __FREE__(statistics_entry->country);
    hash_table_clear(statistics_entry->age_groups_ht);
  }
  __FREE__(statistics_entry);
}
...
char* ptr_to_statistics_entry_serialize(void *v, list_ptr diseases) {
  statistics_entry_ptr statistics_entry = *((statistics_entry_ptr*) v);
  /*
  ...
  */
  ...
  char *serialized_statistics_entry = __MALLOC__(serialized_statistics_entry_size, char);
  ...
  for (size_t i = 1U; i <= list_size(diseases); ++i) {
    list_node_ptr list_node = list_get(diseases, i);
